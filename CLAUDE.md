...
- **Template source of truth**: `Sabre & Badlands Initial Request List.docx` at `https://thebadlandssecuritycompany.sharepoint.com/sites/TheBadlandsDrive/Shared Documents/OneDrive - Tucker's Farm/Sabre & Badlands Initial Request List.docx`. Read it fresh each time rather than hardcoding the text below, in case it's revised. Do NOT save the generated file back into that folder, and do not confuse it with the older `Badlands_Initial Request List.docx` copies scattered under `Admin & Ops/NDA and Request List/` — those are superseded; this Sabre-filled file is the one to read going forward. Content-wise it's the same four-section structure below (it's the "Sabre" deal's filled copy, which happens to still read as the generic template since Sabre's specific answers were captured elsewhere) — see Delivery below for where the generated file goes.
